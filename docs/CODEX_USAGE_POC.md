...
## Windows 0.1.5 live-provider and runtime follow-up (2026-07-31)

The `0.1.5` self-contained x64 publish was exercised again on the same
Windows 10 Home 22H2 (build 19045) two-monitor environment. The runtime probe
passed in both editing and locked click-through modes at 100% scaling. It
confirmed Topmost, ToolWindow, Layered, NoActivate, foreground preservation,
mode-correct click-through, and clean process exit; the locked widget was
captured above another desktop application.

The local `LiveCodexUsageProvider` was also run against the installed Codex App
Server without reading or emitting credentials. At 2026-07-31 21:29 KST it
returned a signed-in account, a weekly bucket at 43% used with reset at
2026-08-05 21:23:07 KST, and no short-term bucket in the response. The provider
therefore rendered the short-term value as unavailable rather than inventing a
zero value. The weekly-history early-reset and normal-reset cases are covered
by 10 focused `UsageHistoryServiceTests`.

The current official Usage-page comparison remains unverified. Direct
navigation to `chatgpt.com` is prohibited by this environment's browser
security policy, so no browser, cookie, storage, or alternative-route
