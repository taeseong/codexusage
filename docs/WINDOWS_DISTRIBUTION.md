...
powershell -ExecutionPolicy Bypass -File scripts\qa-windows-installer-upgrade.ps1 `
  ...
  -CandidateInstallerPath artifacts\installer\CodexUsage-Setup-0.1.5-win-x64.exe
```
...
Pushing a version tag such as `v0.1.5` runs `.github/workflows/release-windows.yml`.
