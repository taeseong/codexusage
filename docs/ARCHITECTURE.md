...
첫 조회 결과가 `CodexNotInstalled`이면 Windows 앱은 프로세스당 한 번 설치 안내 창을 표시합니다. 안내 창은 공식 npm 설치 명령을 복사할 수 있게 하고, 설치 후 `다시 확인`으로 live provider를 재호출합니다. CLI를 찾으면 창을 자동으로 닫으므로 앱을 재실행할 필요가 없습니다. 앱이 Codex CLI를 자동 설치하거나 인증 파일을 직접 읽지는 않습니다.
